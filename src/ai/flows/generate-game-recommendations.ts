...
const GenerateGameRecommendationsOutputSchema = z.object({
  ...
  const prompt = ai.definePrompt({
    ...
    **ステップ1：思考プロセスとスコアリング**
    ...
    1.  **最優先フィルタ (健全性チェック):**
        *   \`DevelopmentRisk\`が'High'のゲームは、他のすべてのスコアに関わらず、この時点で即座に推薦候補から**完全に除外**してください。これは絶対的なルールです。
    
    2.  **知名度対比評価 (RelativeFameScore) の算出:**
        *   ゲームの\`AvgReviewScore\`（平均レビュー評価）と\`SearchVolume\`（検索ボリューム）を考慮し、以下の基準で\`RelativeFameScore\`を'High', 'Medium', 'Low'のいずれかに分類してください。
        *   **'High' (隠れた名作):** \`AvgReviewScore\`が4.0以上 **かつ** \`SearchVolume\`がデータセット全体の平均値の30%以下の場合。
        *   **'Medium' (良作):** \`AvgReviewScore\`が3.5以上 **かつ** \`SearchVolume\`がデータセット全体の平均値の30%から70%の範囲にある場合。
        *   **'Low' (有名作/低評価作):** \`SearchVolume\`がデータセット全体の平均値の90%以上を占める（非常に有名）、または上記'High'/'Medium'の基準を満たさない低評価のゲームの場合。
    
    3.  **ニッチ度・継続性スコア (NicheSatisfaction) の算出:**
        *   このスコアは「一過性のブームではなく、ユーザーが長くプレイし続ける魅力」を測る指標です。
        *   \`AveragePlaytimeHrs\`（平均プレイ時間）が、データセット全体のトップ20%に入るゲームに対して、1.0に近い非常に高い\`NicheSatisfaction\`スコアを与えてください。それ以外のゲームには低いスコアを割り当ててください。
    ...
    **ステップ2：推薦アルゴリズムと最終順位決定**
    ...
    1.  **基本優先順位:**
        *   \`RelativeFameScore\`が'High'または'Medium'のゲームを最優先でリストアップします。
        *   この優先リストの中で、さらに\`NicheSatisfaction\`スコアが高いゲームを上位に配置してください。
    
    2.  **有名作の取り扱い (デチューンと例外処理):**
        *   **標準デチューン:** \`RelativeFameScore\`が'Low'のゲーム（有名作）は、原則として推薦リストの最下位に配置するか、最終的な推薦スコアを強制的に50%低下させてください。
        *   **例外：パーフェクトマッチ・ボーナス:**
            *   **条件:** \`RelativeFameScore\`が'Low'のゲームであっても、ユーザーの自然言語クエリ（{{{query}}}）に対して、あなたの内部的な意味解析で**95%以上の確率で完璧に合致する**と判断した場合にのみ、この例外を適用します。
            *   **処理:**
                1.  最終的な推薦スコアに20%のボーナスを加算します。
                2.  推薦理由（\`recommendationReason\`）に、**必ず**以下の定型文を含めてください：「**有名タイトルですが、あなたのクエリに完璧に合致しているため特別に推薦しました。（プレイ済みでしたら申し訳ありません）**」
    
    **ステップ3：最終的な出力フォーマット**
    
    上記すべてのロジックを適用した上で、最終的な推薦リストを生成してください。
    
    *   **言語に関する重要事項:** 生成されるレスポンスは、**必ず全てのフィールド（\`title\`, \`overview\`, \`recommendationReason\`等）が日本語**でなければなりません。もしゲームの公式な日本語タイトルが存在しない場合は、最も一般的で分かりやすい日本語訳を生成してください。概要や理由も、自然で流暢な日本語で記述してください。
    *   必ず実在するゲームを推薦してください。
    *   ゲームのタイトル、ジャンル、概要、価格、開発健全性スコア（\`DevelopmentRisk\`）、そして上記のロジックに基づいた推薦理由（\`recommendationReason\`）を含めてください。
    *   \`storeUrls\`フィールドに、見つかれば公式ストアへのリンクを含めてください。見つからなければ空の配列とします。
